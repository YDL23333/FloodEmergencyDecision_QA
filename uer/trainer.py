...
class Trainer(object):
    ...
    def evaluate(self, gpu_id, loader, model):
        model.eval()
        # loader_iter = iter(loader)
        ...
        with torch.no_grad():
            ...
            print("evaluate overall: loss {:7.2f} ,accuracy {:3.3f}".format(total_loss / total_batch,
    ...
    def report_and_reset_stats(self):
        ...
        print("| {:8d}/{:8d} steps"
              ...
              "| acc: {:3.3f}".format(
                  ...
                  self.total_loss / self.report_steps,
                  self.total_correct / self.total_denominator))
        self.writer.add_scalar('loss', self.total_loss / self.report_steps, self.current_step)
        self.writer.add_scalar('acc', self.total_correct / self.total_denominator, self.current_step)
...
def worker(proc_id, gpu_ranks, args, model):
    ...
    if args.optimizer in ["adamw"]:
        ...
        # custom_optimizer = str2optimizer[args.optimizer](filter(lambda p : p.requires_grad, model.parameters()), lr=args.learning_rate,
        #                                                  correct_bias=False)
    else:
