    ...
    if rank == 0:
        ...
        for i in range(10):
          ...
          with open(args.test_path, mode="r", encoding="utf-8") as f:
              ...
              #读测试数据
              line = f.readlines()
              line = line[args.line].rstrip().split(' ', 2)
              ...
              #根据模板修改
              src = args.tokenizer.convert_tokens_to_ids([CLS_TOKEN] + desc + args.tokenizer.tokenize(line[0])+[SEP_TOKEN]+args.tokenizer.tokenize(line[1])+[SEP_TOKEN])
          ...
          for i in range(args.seq_length - beginning_length):
              ...
              if next_token.item()==args.tokenizer.convert_tokens_to_ids([SEP_TOKEN])[0]:  # 遇到[SEP]则表明response生成结束
                break
